...
def get_image_names(attempt: str, type_files: list, n_images: int = 20) -> dict:
    ...
    """
    image_names = {}
    base_path = f"error_scores_analysis/mapping/{attempt}/total"
...
def process_images(model, image_names_dict, base_path, compression_values):
    """
    Process images, draw predictions and calculate error scores for each compression level.
    """
    metrics = {
        ...
        for img_name in image_names:
            ...
            for qf in compression_values:
                ...
                metrics[type_name][f"compressed{qf}"].append(
                    {
                        "image": img_name,
                        "error_score": float(matches[0]["error_score"]),
...
def analyze_metrics(metrics):
    ...
    for type_name, type_metrics in metrics.items():
        ...
        for category, data in type_metrics.items():
            if data:
                scores = [float(m["error_score"]) for m in data]
                print(f"\n{category} analysis:")
                ...
                # Print worst cases
                worst_cases = sorted(
                    data, key=lambda x: float(x["error_score"]), reverse=True
                )[:3]
...
def create_comparison_grid(image_metrics, type_name, base_path, compression_values):
    ...
    for img_name, results in image_metrics.items():
        ...
        text_height = 30  # Space for error score text
        total_height = (
        ...
        for idx, quality in enumerate(all_qualities):
            ...
            if quality == "GT":
                ...
                error_score = results[quality]["error_score"]
            ...
            if os.path.exists(img_path):
                ...
                # Add error score text
                text = (
                    f"QF: {quality.replace('compressed', '')} Score: {error_score:.3f}"
                )
                cv2.putText(
                    grid,
                    text,
