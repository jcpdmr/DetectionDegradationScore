...
def create_range_stats(df, column, bins, value_column):
    ...
    stats = df_copy.groupby('bin', observed=True)[value_column].agg(['mean', 'std', 'count']).round(3)
...
def create_box_trace(df, bin_column, value_column, name):
    ...
    return go.Box(
        x=df_valid[bin_column],
        y=df_valid[value_column],
        name=name
    )
...
def create_analysis_report(df, output_path):
    ...
    df['error_bin'] = pd.cut(df['error_score'], 
                            bins=error_bins,
                            labels=error_labels)
    ...
    fig.add_trace(
        go.Table(
            ...
            cells=dict(values=[list(error_1_stats.keys()), list(error_1_stats.values())])
        ),
        row=3, col=2
    )
    ...
    stats_dict = {
        'correlations': {
            ...
            'spearman': {'coefficient': spearman_corr, 'p_value': float(p_value_spearman)}
        },
        'error_score_1_statistics': error_1_stats,
        'error_score_ranges': create_range_stats(df, 'error_score', error_bins, 'quality'),
        'quality_ranges': create_range_stats(df, 'quality', quality_bins, 'error_score')
    }
...
# Define file paths
error_scores_run_folder = "quality_range_uniform_20_60_2"
quality_path = 'dataset_attention/train/quality_mapping.json'
error_scores_path = f'error_scores_analysis/{error_scores_run_folder}/train/error_scores.json'
output_path = f'error_scores_analysis/{error_scores_run_folder}/train/quality_error_analysis.html'
