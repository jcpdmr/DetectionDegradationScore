...
class PatchesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load ground truth (JPG)
        gt_img = load_image_for_yolo(os.path.join(self.gt_path, f"{base_name}.jpg"))

        # Randomly choose between distorted or compressed
        if random.random() < 0.5:
            # Distorted images are JPG
            modified_img = load_image_for_yolo(
                os.path.join(self.distorted_path, f"{base_name}.jpg")
            )
        else:
            # Compressed images are JPG
            modified_img = load_image_for_yolo(
                os.path.join(self.compressed_path, f"{base_name}.jpg")
            )

        return {"gt": gt_img, "modified": modified_img, "name": img_name}
...
def create_dataloaders(
    ...
    train_dataset = PatchesDataset(
        base_path, split="train", val_ratio=val_ratio, seed=seed
    )
