...
def load_data(quality_path, ddscores_path):
    """
    Load quality and dd score data from respective JSON files and combine them into a DataFrame.

    Args:
        quality_path: path to the JSON file containing the quality mapping
        ddscores_path: path to the JSON file containing the dd scores

    Returns:
        DataFrame with columns: image, quality, ddscore
    """
    with open(quality_path, "r") as f:
        quality_mapping = json.load(f)

    with open(ddscores_path, "r") as f:
        ddscores = json.load(f)

    data = [
        {
            "image": img_name,
            "quality": quality_mapping[img_name],
            "ddscore": ddscores[img_name],
        }
        for img_name in set(quality_mapping.keys()) & set(ddscores.keys())
    ]
...
def create_analysis_report(df, output_path):
    ...
    analyzing the relationship between quality and dd score.
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=(
            "Scatter Plot: Quality vs DDS",
            "Quality Distribution by DDS Ranges",
            "DDS Distribution by Quality Ranges",
            "Distribution of Quality Values for DDS = 1",
    ...
    z = np.polyfit(df["quality"], df["ddscore"], 1)
    p = np.poly1d(z)
    fig.add_trace(
        go.Scatter(
            x=df["quality"],
    ...
    # 2. Create bins for dd score (range 0-1)
    error_bins = np.arange(0, 1.1, 0.1)
    error_labels = get_bin_labels(error_bins)
    df["error_bin"] = pd.cut(df["ddscore"], bins=error_bins, labels=error_labels)
    ...
    fig.add_trace(
        create_box_trace(df, "quality_bin", "ddscore", "Error by Quality Range"),
        row=2,
        col=1,
    )

    # 4. Analyze cases with ddscore = 1
    perfect_errors = df[df["ddscore"] == 1.0]
    fig.add_trace(
        go.Histogram(x=perfect_errors["quality"], nbinsx=20, name="DDS = 1"),
        row=2,
        col=2,
    )

    # 5. Calculate and display correlations
    pearson_corr, p_value_pearson = stats.pearsonr(df["quality"], df["ddscore"])
    spearman_corr, p_value_spearman = stats.spearmanr(df["quality"], df["ddscore"])
    ...
    # 6. Statistics for cases with ddscore = 1
    error_1_stats = {
    ...
    fig.update_layout(
        ...
        width=1600,
        showlegend=False,
        title_text="Analysis of Quality vs DDS Relationship",
    )
    ...
    stats_dict = {
        ...
        "ddscore_1_statistics": error_1_stats,
        "ddscore_ranges": create_range_stats(df, "ddscore", error_bins, "quality"),
        "quality_ranges": create_range_stats(df, "quality", quality_bins, "ddscore"),
    }
...
# Define file paths
ddscores_run_folder = "09_quality_range_uniform_10_30_visgen"
quality_path = "dataset_attention/train/quality_mapping.json"
ddscores_path = f"ddscores_analysis/{ddscores_run_folder}/train/ddscores.json"
output_path = (
    f"ddscores_analysis/{ddscores_run_folder}/train/quality_error_analysis.html"
)

# Run the analysis
df = load_data(quality_path, ddscores_path)
