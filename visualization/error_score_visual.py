...
def load_ddscores(filepath):
    """
    Load dd scores from a JSON file and return them as a list
    """
...
def create_ddscore_analysis(ddscores, output_path, title_prefix):
    """
    Create comprehensive dd score analysis with multiple visualizations

    Args:
        ddscores: List of dd scores
    ...
    mean_score = np.mean(ddscores)
    median_score = np.median(ddscores)
    std_score = np.std(ddscores)
    percentiles = np.percentile(ddscores, [25, 50, 75])
    ...
    fig.add_trace(
        go.Histogram(x=ddscores, nbinsx=20, name="DD scores", showlegend=False),
        row=1,
        col=1,
    )
    ...
    qq = scipy_stats.probplot(ddscores)
    fig.add_trace(
        go.Scatter(
            x=qq[0][0],
            y=qq[0][1],
    ...
    sorted_scores = np.sort(ddscores)
    ...
    fig.update_layout(
        height=1000,
        width=1200,
        title_text=f"{title_prefix} DDS Analysis",
    ...
    fig.update_yaxes(title_text="DDS", row=1, col=2)
...
train_scores = load_ddscores("balanced_dataset/train/ddscores.json")
val_scores = load_ddscores("balanced_dataset/val/ddscores.json")

# Create visualizations and get statistics
train_fig, train_stats = create_ddscore_analysis(
    train_scores,
    "balanced_dataset/train/ddscores_visual.html",
    "Training",
)

val_fig, val_stats = create_ddscore_analysis(
    val_scores,
    "balanced_dataset/val/ddscores_visual.html",
    "Validation",
)
