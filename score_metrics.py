...
def match_predictions(
    ...
    for gt_pred, mod_pred in zip(gt_predictions, mod_predictions):
        ...
        image_metrics = {
            ...
            "error_score": calculate_quality_score(
                matches, len(gt_boxes), len(mod_boxes)
            ),
        }
...
def calculate_quality_score(matches: List[Dict], num_gt: int, num_mod: int) -> float:
    """
    Calculate quality score based on matches and predictions.
    ...
    Returns:
        Error score between 0 (perfect match) and 1 (complete mismatch)
    """
    # Handle edge cases
    ...
    quality_score = sum(match_qualities) / max(num_gt, num_mod)
