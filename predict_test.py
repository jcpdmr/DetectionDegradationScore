...
def predict_test_set(
    ...
    if enable_bench_time and torch.cuda.is_available():
        ...
        print(f"Baseline GPU memory - Allocated: {baseline_allocated:.2f} MB, Reserved: {baseline_reserved:.2f} MB")
    ...
    if enable_bench_time and torch.cuda.is_available():
        ...
        print(f"Model loading memory overhead - Allocated: {model_memory_overhead_allocated:.2f} MB, Reserved: {model_memory_overhead_reserved:.2f} MB")
        ...
        for batch_idx, batch in enumerate(tqdm(test_loader, desc="Making predictions")):
            ...
            if enable_bench_time and batch_idx >= warmup_batches:
                ...
                if batch_idx == warmup_batches and torch.cuda.is_available():
                    current_allocated, current_reserved = get_gpu_memory_usage(device_id)
                    inference_memory_allocated = current_allocated - model_loaded_allocated
                    inference_memory_reserved = current_reserved - model_loaded_reserved
                    print(f"Inference memory usage - Allocated: {inference_memory_allocated:.2f} MB, Reserved: {inference_memory_reserved:.2f} MB")
    ...
    if enable_bench_time and total_images > 0:
        ...
        if torch.cuda.is_available():
            print("\nMemory Usage:")
            print(f"Model loading overhead - Allocated: {model_memory_overhead_allocated:.2f} MB, Reserved: {model_memory_overhead_reserved:.2f} MB")
            print(f"Inference memory usage - Allocated: {inference_memory_allocated:.2f} MB, Reserved: {inference_memory_reserved:.2f} MB")
            print(f"Peak memory usage - Allocated: {peak_allocated:.2f} MB, Reserved: {peak_reserved:.2f} MB")
    ...
    quality_metrics = ["number_of_predictions", "average_predicted_distance", "MAE", 
